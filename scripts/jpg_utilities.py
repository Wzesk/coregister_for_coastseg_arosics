# In this file many of these functions are code from CoastSat. They have been modified for our purposes. Credit and thanks to the original authors.
import os
import re
import numpy as np
...
def save_single_jpg(
    ...
    filepath_data,
    collection,
    pansharpen_all_bands:bool = True,
    **kwargs,
):
    """
    ...
    if pansharpen_all_bands:
        im_ms, georef, cloud_mask, im_extra, im_QA, im_nodata = preprocess_single_modified(
            fn, satname, cloud_mask_issue, False, collection, do_cloud_mask
        )       
    elif not pansharpen_all_bands:
...
def preprocess_single_modified(
    fn, satname, cloud_mask_issue, pan_off, collection='C02', do_cloud_mask=True, s2cloudless_prob=60
):
    """
    This is a modified version of the preprocess_single function from CoastSat. This function is only different in that
    it pansharpens all of the bands for Landsat 7, 8, and 9. 
